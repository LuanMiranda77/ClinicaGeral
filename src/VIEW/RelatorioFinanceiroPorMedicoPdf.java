...
public class RelatorioFinanceiroPorMedicoPdf {
	private static DecimalFormat convertMoeda = new DecimalFormat("#0.00");
	

	public static void gerarHistorico(ResultSet res,Profissional medico,String inicio,String fim)throws DocumentException, ParseException, IOException {
		double total=0;
		int quant=0;
		...
		Paragraph titulo1 = new Paragraph(new Phrase(20F, "Relatório das Consultas", fonte));
		...
		// dados clientes
				Paragraph cliente0 = new Paragraph(new Phrase(20F, "DADOS DO MEDICO:", fontCliente));
				cliente0.setAlignment(Element.ALIGN_LEFT);
				boleto.add(cliente0);
				
				Font f5 = new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.BOLD, BaseColor.BLACK);
				Paragraph cliente1 = new Paragraph(new Phrase(20F, "Medico: " + medico.getNome(), f4));
				cliente1.setAlignment(Element.ALIGN_LEFT);
				boleto.add(cliente1);
				
				
				Paragraph cliente2 = new Paragraph(new Phrase(20F, "CRM: " + medico.getMatricula()+"\n\n", f5));
				cliente2.setAlignment(Element.ALIGN_LEFT);
				boleto.add(cliente2);
		...
		PdfPCell celula1 = new PdfPCell(new Phrase(20F,"Consultas Realizadas "+inicio+" até "+fim ,fontCliente));
